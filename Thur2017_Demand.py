...
cursor.execute('''
SELECT Time.Hour AS Thursday_2017_Hours, AVG(CarSharing.Demand) AS Average_Demand
FROM Time
INNER JOIN CarSharing ON Time.Timestamp = CarSharing.Timestamp
WHERE CarSharing.Timestamp >= '2017-01-01' AND CarSharing.Timestamp < '2018-01-01' AND Time.Weekday = 'Thursday'
GROUP BY Time.Hour 
